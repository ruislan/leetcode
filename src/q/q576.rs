...
impl Solution {
    pub fn find_paths(m: i32, n: i32, max_move: i32, start_row: i32, start_column: i32) -> i32 {
        ...
        // AC 0ms 2.5mb
        use std::collections::HashMap;
        fn dfs(x: i32, y: i32, s: i32, m: i32, n: i32, max_move: i32, mem: &mut Vec<Vec<Vec<i32>>>) -> i32 {
            if s > max_move { return 0; }
            if x < 0 || y < 0 || x >= m || y >= n { return 1; }
            if mem[x as usize][y as usize][s as usize] != -1 { return mem[x as usize][y as usize][s as usize]; }
            let mut ret = dfs(x + 1, y, s + 1, m, n, max_move, mem) as i64 +
                dfs(x - 1, y, s + 1, m, n, max_move, mem) as i64 +
                dfs(x, y + 1, s + 1, m, n, max_move, mem) as i64 +
                dfs(x, y - 1, s + 1, m, n, max_move, mem) as i64;
            ret = ret % 1000000007;
            let ret = ret as i32;
            mem[x as usize][y as usize][s as usize] = ret;
            ret
        }

        dfs(start_row, start_column, 0, m, n, max_move, &mut vec![vec![vec![-1; max_move as usize + 1]; n as usize]; m as usize])
    }
}